    ...
    loadUserFavResource(){
        ...
        if(isLogin()){
            getUserResource(getUser().id).then((result)=>{
                let tempArr = [];
                if(result && result.status === 1){
                    tempArr = result.data.map((item)=>{return item.resource_id});
                    this.setState({
                        user_resource_id: tempArr
                    }, ()=>{
                       console.log(this.state.user_resource_id)
                    });
                }
            }).catch((error)=>{
                console.log(error);
                ...
                isUserBuyResource(getUser().id, resource.id).then((result)=>{
                    ...
                    if(result && result.status === 1){ // 已经购买
                        ...
                        Modal.confirm({
                            ...
                            content: `该资源的价格是¥${resource.resource_price}元!`,
                            okText: '确认下单',
                            cancelText: '我在想想',
                            onOk: ()=>{
                ...
                <div className="mainwraper">
                    ...
                    <div className="content content01y clearfix">
                        <div className="mainbox clearfix">
                            ...
                            <div className="sort_right">共有<span className="lm_desc_num">{resource_list_count}</span>条资源，供您选择学习</div>
                                            ...
                                            <div className="resource_con" key={item.id}>
                                               <div onClick={()=>this.toResourceDetail(item)}>
                                                   ...
                                                   <div className="resource_con_other">
                                                       ...
                                                       <span className="resource_con_other_price">￥{item.resource_price}.00</span>
                                                   </div>
                                               </div>
                                            </div>
                                        )
